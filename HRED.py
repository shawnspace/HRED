...
def impl(input_features,mode,hp):
    ...
    response_in = input_features['response_in'] # batch_size,max_res_length(with eos token)
    response_out = input_features['response_out'] #batch_size, max_res_length (with eos token append before)
    response_mask = input_features['response_mask'] #batch_size, max_res_length (with eos token append before)
    ...
    with tf.variable_scope('word_encoder_layer') as vs:
    ...
    with tf.variable_scope('context_encoder_layer') as vs:
        kernel_initializer = tf.random_normal_initializer(mean=0.0, stddev=0.01, seed=random_seed)
        bias_initializer = tf.zeros_initializer()
        ...
        if mode == modekeys.TRAIN:
            initialial_state = context_cell.zero_state(batch_size=hp.batch_size,dtype=tf.float32)
        else:
            initialial_state = context_cell.zero_state(batch_size=hp.eval_batch_size,dtype=tf.float32)
    ...
    with tf.variable_scope('decoder_layer') as vs:
        if mode == modekeys.TRAIN:
            sequence_length = tf.constant(value=hp.max_response_length,dtype=tf.int32,shape=[hp.batch_size])
            helper = tf.contrib.seq2seq.TrainingHelper(inputs=response_in, sequence_length=sequence_length)
        elif mode == modekeys.EVAL:
            sequence_length = tf.constant(value=hp.max_response_length, dtype=tf.int32, shape=[hp.eval_batch_size])
            helper = tf.contrib.seq2seq.TrainingHelper(inputs=response_in, sequence_length=sequence_length)
        else:
            helper = None # beam search doesn't need helper
        ...
        if mode == modekeys.TRAIN:
            decoder = tf.contrib.seq2seq.BasicDecoder(cell=decoder_cell,
                                                      helper=helper,
                                                      initial_state=context_encoding,output_layer=output_layer)
            ...
            cross_entropy = tf.multiply(cross_entropy,tf.to_float(response_mask))
            loss = tf.reduce_sum(cross_entropy,axis=1) # shouldn't divide num_steps
            loss = tf.reduce_mean(loss)
            return loss,final_outputs.sample_id,final_sequence_lengths,response_out
            ...
            cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=response_out, logits=logit)
            cross_entropy = tf.reduce_mean(tf.multiply(cross_entropy, tf.to_float(response_mask)))
            ppl = tf.exp(cross_entropy)
            return ppl

        elif mode == modekeys.PREDICT:
            tiled_context_encoding = tf.contrib.seq2seq.tile_batch(context_encoding, multiplier=hp.beam_width)
            decoder = tf.contrib.seq2seq.BeamSearchDecoder(
                cell=decoder_cell,
                embedding=embedding_w,
                start_tokens=tf.constant(value=1,dtype=tf.int32,shape=[hp.eval_batch_size]),
                end_token=1,
            ...
            return final_outputs.predicted_ids, final_sequence_lengths #[batch_size, T, beam_width] T is max(final_sequence_lengths); final_sequence_lengths:batch,beam_width
