...
def batch_prediction(datafile,model_dir):
    hp = hparam.create_hparam()

    eval_graph = tf.Graph()
    with eval_graph.as_default():
        input_features = input_layer.create_input_layer(filename=[datafile],hp=hp,mode=modekeys.EVAL)
...
if __name__ == '__main__':
    ...
    batch_prediction('./twitter_data/train.tfrecords','./model/twitter1')
